...
typedef struct PCB {
  ...
  //uint32	pagetable[256]; // Statically allocated page table
  uint32	pagetable[4]; // Statically allocated page table
  Link		*l;		// Used for keeping PCB in queues
  uint32 npages; //Used to keep track of number of pages allocated.
} PCB;

extern PCB	*currentPCB;
uint32 l2pagetable[256][64];
int allocated_l2;
