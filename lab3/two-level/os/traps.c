	     ...
	     uint32 *trapArgs)
{
  ...
  if (cause & TRAP_TRAP_INSTR) {
    ...
    case TRAP_PROCESS_FORK:
      printf("Entered the fork handler.\n");
      //Takes the currentPCB as an input to copy the data.
      forkPid = ProcessRealFork(currentPCB);
      dbprintf ('t', "Got a fork trap!\n");
      printf("Returned pid from fork() = %d.\n",forkPid);
      ProcessSetResult(currentPCB,forkPid);
      //ProcessSetResult(currentPCB,0);
      break;
    ...
    case TRAP_ACCESS:
      ...
      printf ("Illegal virtual address access.\n");
      printf ("Killing the process with pid = %d\n",GetCurrentPid());
      //ProcessKill();
      exitsim ();
      break;
    case TRAP_ADDRESS:
    ...
    case TRAP_ILLEGALINST:
      ...
      break;
    case TRAP_PAGEFAULT:
      ret = MemoryPageFaultHandler(currentPCB);
      if(ret == MEM_SUCCESS)
        printf("Successfully allocated page.\n");
      else
      {
          printf("PageFaultHandler Failed.\n");
          ProcessKill();
      }
    ...
    case TRAP_ROP_ACCESS:
      printf("ROP handler : Trying to modify a read-only page.\n");
