...
extern uint32 l2pagetable[256][64];
extern int allocated_l2;
...
void ProcessModuleInit () {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    //-------------------------------------------------------
    dbprintf('m',"Initializing pcb pagetable.\n");
  ...
  for( i = 0; i < MEM_L1TABLE_SIZE; i++)
  {
    //for( j = 0; j < MEM_L2TABLE_SIZE; j++)
    {
      if((pcb->pagetable[i] != 0))
      {
        ...
        //dbprintf('m',"ProcessFreeResources() : pcb->pagetable[%d]=0x%x\n",i,pcb->pagetable[i]);
        //MemoryFreePage((*(pcb->pagetable + i )) >> 12);
        dbprintf('r',"ProcessFreeResources() : pcb->pagetable[%d] = 0x%x\n",i,pcb->pagetable[i]);
        MemoryFreeL2Table((pcb->pagetable[i]));
      }
    }
    
      
      //dbprintf('r',"ProcessFreeResources() : Freeing pagenumber : %d\n",pcb->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM);
  //MemoryFreePage (pcb->pagetable[0] >> MEM_L1FIELD_FIRST_BITNUM);
  }
  //Freeing systemPage
  dbprintf('r',"The PTE at pcb->sysStackArea = 0x%x\n",pcb->sysStackArea);
  MemoryFreePage(pcb->sysStackArea >> 12);
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  if(retVal == 0)
  {
    printf("ProcessFork() : No memory available.\n");
    return -1;
  }
  dbprintf('m', "ProcessFork() : Retval of MemoryAllocPage() = %d\n",retVal);

  sysStackPage = MemorySetupPte(retVal);
  stackframe = (sysStackPage - 1) + MEM_PAGESIZE - 4;
  dbprintf('m',"STack Frame Address = 0x%x\n",stackframe);
  //pcb->npages += 1;
  ...
  for( i = 0; i < 4; i++)
  {
    ...
    if(retVal != 0 )
    {
      ...
      l2pagetable[allocated_l2][i] = MemorySetupPte(retVal);
      //dbprintf('m',"ProcessFork() : *(pcb->pagetable + [%d]) = 0x%x\n",i, *(pcb->pagetable + i));
      pcb->npages += 1;
    }
    else
    {
      printf("Process Fork() : Memory not available.\n");
      return -1;
    }
  }
  pcb->pagetable[0] = &l2pagetable[allocated_l2][0]; 
  allocated_l2 += 1;
  ...
  dbprintf('m',"STACKFRAME size = 0x%x(Hex) in dec = %d\n",PROCESS_STACK_FRAME_SIZE,PROCESS_STACK_FRAME_SIZE);
  ...
  // previous frame.
  dbprintf('r', "ProcessFork: stackframe = 0x%x\n", (int)stackframe);
  dbprintf ('r',
  ...
  // stack frame.
  //----------------------------------------------------------------------
  dbprintf('r',"ProcessFork() : Base address of page table = %p\n",pcb->pagetable);
  ...
  dbprintf('m',"ProcessFork() : Base address of lv 1 page table = 0x%x\n",stackframe[PROCESS_STACK_PTBASE]);
  dbprintf('m',"ProcessFork() : Size of page table = %d\n",stackframe[PROCESS_STACK_PTSIZE]);
  dbprintf('m',"ProcessFork() : PROCESS_STACK_PTBITS = %x\n",stackframe[PROCESS_STACK_PTBITS]);
  //exit();

  dbprintf('m',"wait here.\n");
  if (isUser) {
    ...
    l2pagetable[allocated_l2][63]= MemorySetupPte(retVal);
    pcb->pagetable[3] = &l2pagetable[allocated_l2][0];
    //printf("Ranga()()() : size of MEM_L1_Table_size -1 = 256\n");
    ...
    dbprintf('r',"ProcessFork() : stackframe[PROCESS_STACK_USER_STACKPOINTER] = %x\n", stackframe[PROCESS_STACK_USER_STACKPOINTER]);
    pcb->npages += 1;
    //dbprintf('m',"ProcessFork() : pcb->pagetable[%d][%d] = 0x%x\n",MEM_L2TABLE_SIZE - 1, MEM_L1TABLE_SIZE,pcb->pagetable[MEM_L2TABLE_SIZE - 1][MEM_L1TABLE_SIZE - 1]);
    printPageTable(pcb);
    dbprintf('r',"ProcessFork() : stackframe[PROCESS_STACK_USER_STACKPOINTER] = %x\n", stackframe[PROCESS_STACK_USER_STACKPOINTER]);
    allocated_l2 += 1;

    //printf("ProcessFork() : allocated_l2 = %d\n",allocated_l2);
  ...
  //PrintPPageCounters();
  //exit();
  dbprintf ('r', "Leaving ProcessFork (%s)\n", name);
...
void printPageTable(PCB* pcb)
{
  int i = 0, j= 0;
  int *l2base;
  dbprintf('m', "PrintPageTable of PCB %p\n",pcb);
  ...
  for(i = 0; i < MEM_L1TABLE_SIZE; i++)
  {
    if(pcb->pagetable[i] != 0 )
    {
      l2base = pcb->pagetable[i];
      dbprintf('m',"l2pagetable base at pcb->pagetable[%d] = 0x%x\n",i,pcb->pagetable[i]);
      for( j= 0; j < MEM_L2TABLE_SIZE;j++)
      {
        dbprintf('m',"pagetable[%d]= 0x%x \n",j,l2base[j]);
      }
    }
  //exit();
  }
...
{
  dbprintf('r',"PCB COntent() : Address of current pcb = %p\n",pcb);
  dbprintf('r',"PCB COntent() : Address of current saved frame = %p\n",&pcb->currentSavedFrame);
  dbprintf('r',"PCB COntent() : Value of current saved frame = %p\n",pcb->currentSavedFrame);
  dbprintf('r',"PCB COntent() : Address of  sysStackPtr = %p\n",&pcb->sysStackPtr);
  dbprintf('r',"PCB COntent() : Value of  sysStackPtr = 0x%x\n",(pcb->sysStackPtr));
  dbprintf('r', "PCB Content() : Page table base address = %p\n",pcb->pagetable);
  dbprintf('r', "PCB Content() : PTE-0 = %p and PTE entry = 0x%x\n",pcb->pagetable,pcb->pagetable[0]);
  dbprintf('r'," PCB COntent() : PTE-1 = %p and PTE Entry = 0x%x\n",pcb->pagetable + sizeof(int),pcb->pagetable[1]);
}
...
//--------------------------------------------------------------------------
void ProcessKill() {
  dbprintf('r', "ProcessKill: killing processid %d\n", GetCurrentPid());
...
//   int offset;
//   int sysPage;
//   int* old_syspointer, new_syspointer;

//   printf("Entered the ProcessREalFork() function.\n");
//   //PrintPPageCounters(); 
//   intrs = DisableIntrs ();
//   dbprintf ('I', "Old interrupt value was 0x%x.\n", intrs);
//  // dbprintf ('p', "Entering ProcessFork args=0x%x 0x%x %s %d\n", (int)func,
...
//   //Copying one PCB to another.
//   bcopy((char*)currentPCB, (char*)pcb,sizeof(PCB));
  

//   for( i = 0; i < MEM_L1TABLE_SIZE; i++)
//   {
//     if(currentPCB->pagetable[i] != 0 )
//     {
//       currentPCB->pagetable[i] = currentPCB->pagetable[i] | MEM_PTE_READONLY;
//       pcb->pagetable[i] = currentPCB->pagetable[i];

//       PPageNum = currentPCB->pagetable[i] >> MEM_L1FIELD_FIRST_BITNUM;
//       printf("RealFork() : CurrentPCB->pagetable[%d] = 0x%x , pcb->pagetable[%d] = 0x%x\n",i, currentPCB->pagetable[i], i, pcb->pagetable[i]);
//       pagerow = PPageNum / 32;
//       printf("RealFork() : pagerow = %d\n",pagerow);
//       offset = PPageNum % 32;
//       printf("RealFork() : offset = %d\n",offset);
...
//       return -1;
//     }
//   pcb->pagetable[MEM_L1TABLE_SIZE - 1] = MemorySetupPte(retVal);
//   stackframe[PROCESS_STACK_USER_STACKPOINTER] = MEM_MAX_VIRTUAL_ADDRESS - 3;
//   //dbprintf('m', "stackframe[PROCESS_STACK_USER_STACKPOINTER] = %x\n", stackframe[PROCESS_STACK_USER_STACKPOINTER]);
//   dbprintf('r',"stackframe[PROCESS_STACK_USER_STACKPOINTER] = %x\n", stackframe[PROCESS_STACK_USER_STACKPOINTER]);
//   pcb->npages += 1;
//   PrintPPageCounters(); 
//   */
...
//   sysPage = MemorySetupPte(retVal);
//   stackframe = (sysPage - 1) + MEM_PAGESIZE - 4;
//   dbprintf('r',"STack Frame Address = 0x%x\n",stackframe);
//   pcb->npages += 1;
...
//   // initial stack frame that will be loaded for this PCB when it gets switched in by 
//   // ProcessSchedule the first time.
//   dbprintf('r',"STACKFRAME size = 0x%x(Hex) in dec = %d\n",PROCESS_STACK_FRAME_SIZE,PROCESS_STACK_FRAME_SIZE);
//   stackframe -= PROCESS_STACK_FRAME_SIZE;
//   // The system stack pointer is set to the base of the current interrupt stack frame.
//   pcb->sysStackPtr = stackframe;
//   // The current stack frame pointer is set to the same thing.
//   pcb->currentSavedFrame = stackframe;
//   pcb->sysStackArea = sysPage;


//   old_syspointer = currentPCB->sysStackArea;
//   new_syspointer = pcb->sysStackArea;
//   printf("Copying the system stack.\n");  
...
//   dbprintf ('r',
// 	    "Setting up PCB @ 0x%x (sys stack=0x%x, mem=0x%x, size=0x%x)\n",
// 	    (int)pcb, pcb->sysStackArea, sysPage,
// 	    MEM_PAGESIZE);
//   stackframe[PROCESS_STACK_PREV_FRAME] = 0;
...
//   //Implemet the system stack portion.
//   //pcb->sysStackPtr = (*(currentPCB->sysStackPtr) & ((1 << MEM_PAGESIZE) - 1)) |  (pcb->pagetable[0]);
//   printPCBContent(currentPCB);
//   printPCBContent(pcb);
