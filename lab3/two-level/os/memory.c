...
uint32 MemoryTranslateUserToSystem (PCB *pcb, uint32 addr) {
  ...
  uint32 l2_pagenum = (addr >> MEM_L2FIELD_FIRST_BITNUM) & ((1 << 6) - 1);
  uint32 offset = addr & (MEM_ADDRESS_OFFSET_MASK);
  uint32* l2pagetablebase = pcb->pagetable[l1_pagenum];
  //uint32 physicalAddress = (*(pcb->pagetable + (addr >> 12))) & ~(MEM_ADDRESS_OFFSET_MASK) | offset;
  uint32 physicalAddress = (l2pagetablebase[l2_pagenum] & ~(MEM_ADDRESS_OFFSET_MASK)) | offset;
  dbprintf('m',"MemoryTranslate() : vaddr = 0x%x\n",addr);
  
  dbprintf('m',"MemoryTranslate() : l1_num = %d, l2_num = %d\n",l1_pagenum,l2_pagenum);
  //dbprintf('m',"MemoryTranslate() : Address at that l2 and l1 = 0x%x\n",pcb->pagetable[l2_pagenum][l1_pagenum]);
...
int MemoryPageFaultHandler(PCB *pcb) {
  ...
  if (addr >= stackPointerPage)
  {
    ...
    l2pagetable = pcb->pagetable[3];
    l2pagetable[(addr >> 12) & ((1 << 6) - 1)] = MemorySetupPte(ppagenum); 
    //printf("Allocated a page with l2 num = 0x%x li page = 0x%x\n",addr>>14,(addr >> 12) & 0xff);
    //printf("Allocated page base address = 0x%x\n",pcb->pagetable[(addr >> 14)* 4 +  ((addr >> 12) & 0xff)]);
    //printf("Ppagenum = %d\n",ppagenum);
    //printf("The address of l2 page table = 0x%x\n",l2pagetable);
    //printf("STackPointerPage >> 12 = 0x%x\n",stackPointerPage >> 12);
    printf("PTE at l2[%d] = 0x%x\n",(stackPointerPage >> 12) & ((1 << 6) - 1),l2pagetable[(stackPointerPage >> 12) & ((1 << 6) - 1)]);
...
void MemoryFreeL2Table(uint32* baseaddress)
{
  int i = 0, j =0;
  dbprintf('r',"MemoryFreeL2() : Obtained address = 0x%x\n",baseaddress);
  for( i = 0; i < MEM_L2TABLE_SIZE; i++)
  {
    if((baseaddress[i] != 0))
    {
      dbprintf('r',"MemoryFreePage() : baseaddress[%d] = 0x%x\n",i,baseaddress[i]);
      MemoryFreePage(baseaddress[i] >> 12);
    }
  }
  allocated_l2 -= 1;
  dbprintf('r',"MemoryFreePage() : Allocated l2 = %d\n",allocated_l2);
...
void MemoryFreePage(uint32 page) {
  ...
  dbprintf('r',"MemoryFreePage() Before : freemap[%d] = %x\n",numPage,freemap[numPage]);
  ...
  dbprintf('r',"MemoryFreePage() : PpageCounters[numPage][offset] = %d\n",PpageCounters[numPage][offset]);
}
...
void MemoryInfo()
{
  ...
  dbprintf('r', "L2 Page Table Size = %d, in Hex = %x\n",MEM_L2TABLE_SIZE, MEM_L2TABLE_SIZE);
  dbprintf('r', "L1 Page Table Size = %d, in Hex = %x\n",MEM_L1TABLE_SIZE, MEM_L1TABLE_SIZE);
  dbprintf('r', "Max pages = %d, in Hex = %x\n",MEM_MAX_PAGES,MEM_MAX_PAGES);
...
void ROPAccessHandler(PCB *pcb)
{
  ...
  int i = 0;
  int newpagetable;
  int l2_page = pcb->currentSavedFrame[PROCESS_STACK_FAULT] >> MEM_L2FIELD_FIRST_BITNUM;
  int l1_page = (pcb->currentSavedFrame[PROCESS_STACK_FAULT] >> MEM_L1FIELD_FIRST_BITNUM) & ((1 << (MEM_L2FIELD_FIRST_BITNUM - MEM_L1FIELD_FIRST_BITNUM)) - 1);
