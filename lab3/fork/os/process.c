...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  for( i = 0; i < 4; i++)
  {
    ...
    if(retVal != 0 )
    {
      ...
      pcb->pagetable[i] = MemorySetupPte(retVal);
...
// Now that the stack frame points at the bottom of the system stack memory area, we need to
  ...
  bcopy((char*)old_syspointer,(char*)new_syspointer,MEM_PAGESIZE);
  //bcopy((char*)&currentPCB->sysStackArea,(char*)&pcb->sysStackArea,MEM_PAGESIZE);
  /*for(i = 0; i < MEM_PAGESIZE; i++)
  //bcopy((char*)&currentPCB->sysStackArea,(char*)&pcb->sysStackArea,MEM_PAGESIZE);
  {
    //*(new_syspointer++ ) = *(old_syspointer++);
    printf("Address of old_syspointer = 0x%x and the the data %d",old_syspointer);
  }*/
  //exit();
 ...
 // stackframe[PROCESS_STACK_PTSIZE] = MEM_L1TABLE_SIZE;
  ...
  printf("Printing the parent page table after fork()\n");
  PrintValidPages(currentPCB);
  printf("Printing the child page table after fork()\n");
  PrintValidPages(pcb);
  return (int)(pcb - pcbs) ; 
  //ProcessSchedule();
}
...
void PrintValidPages(PCB* pcb)
{
    ...
    for(i = 0; i < MEM_L1TABLE_SIZE; i++)
    {
        if(pcb->pagetable[i] != 0)
        {
            printf("The pid = %d, pagetable[%d] = 0x%x\n",pcb - pcbs, i, pcb->pagetable[i]);
        }
    }
}
