...
int TrapFileLinkHandler(uint32 *trapArgs, int sysMode)
{
  char filename[FILE_MAX_FILENAME_LENGTH2];
  char source_filename[FILE_MAX_FILENAME_LENGTH2];
  char destination_filename[FILE_MAX_FILENAME_LENGTH2];
  int i;
  char* userfilename1 = NULL;
  char* userfilename2 = NULL;
  printf("Entered the actual handler.\n");
  bzero(source_filename,sizeof(source_filename));
  bzero(destination_filename,sizeof(destination_filename));
  printf("After bzero\n");
  // If we're not in system mode, we need to copy everything from the
  // user-space virtual address to the kernel space address
  if (!sysMode) {
    ...
    MemoryCopyUserToSystem (currentPCB, (trapArgs+0), &userfilename1, sizeof(uint32));
    MemoryCopyUserToSystem (currentPCB, (trapArgs+1), &userfilename2, sizeof(uint32));
  
    // Now copy userland filename string into our filename buffer
    for(i=0; i<FILE_MAX_FILENAME_LENGTH2; i++) {
      MemoryCopyUserToSystem(currentPCB, (userfilename1+i), &(source_filename[i]), sizeof(char));
      // Check for end of user-space string
      if (source_filename[i] == '\0') break;
    }
     for(i=0; i<FILE_MAX_FILENAME_LENGTH2; i++) {
      MemoryCopyUserToSystem(currentPCB, (userfilename2+i), &(destination_filename[i]), sizeof(char));
      // Check for end of user-space string
      if (destination_filename[i] == '\0') break;
    }
    if (i == FILE_MAX_FILENAME_LENGTH2) {
      printf("TrapFileRmDirHandler: length of filename longer than allowed!\n");
      GracefulExit();
    ...
    // Already in kernel space, no address translation necessary
    bcopy((char *)(trapArgs[0]), source_filename, FILE_MAX_FILENAME_LENGTH2);  
    bcopy((char *)(trapArgs[1]), destination_filename,  FILE_MAX_FILENAME_LENGTH2);  

  }
  dbprintf('F', "TrapFileRmDirHandler: calling RmDir(\"%s\")\n", filename);
  return FileLink(source_filename,destination_filename);
}
	     ...
	     uint32 *trapArgs)
{
  ...
  if (cause & TRAP_TRAP_INSTR) {
    ...
    case TRAP_FILE_RMDIR:
      ...
      break;
    case TRAP_FILE_LINK:
        printf("Entered the trap handler for file_link\n");
        ProcessSetResult(currentPCB, TrapFileLinkHandler(trapArgs, isr & DLX_STATUS_SYSMODE));
        break;
