...
sem_t SemCreate(int count) {
  ...
  //printf("OS SemCreate : sem->count = %d\n",count);
  return sem;
}
...
int SemWait (Sem *sem) {
  ...
  RestoreIntrs (intrval);
  //printf("OS Sem_wait : sem->count = %d\n",sem->count);
  return SYNC_SUCCESS;
}
...
int SemSignal (Sem *sem) {
  ...
  RestoreIntrs (intrs);
  //printf("OS Sem_signal : sem->count = %d\n",sem->count);
  return SYNC_SUCCESS;
}
...
int LockAcquire(Lock *k) {
  ...
  //printf("Lock acquire function now.\n");
  // Locks are atomic
  intrval = DisableIntrs ();
  dbprintf ('x', "LockAcquire: Old interrupt value was 0x%x.\n", intrval);
  ...
  dbprintf ('x', "LockAcquire: Proc %d asking for lock %d.\n", GetCurrentPid(), (int)(k-locks));
...
int LockRelease(Lock *k) {
  ...
  if (!k) return SYNC_FAIL;
  //printf("ENtered the LockRelease function.\n");
  intrs = DisableIntrs ();
...
int CondInit(Cond* cond)
{
  if(!cond)
  {
    printf("COnd value is NULL. CondInit failed.\n");
    return SYNC_FAIL;
  }
  if(AQueueInit(&cond->waiting) != QUEUE_SUCCESS)
  {
    printf("FATAL ERROR : could not initialize waiting queue for cond var in Cond Init.\n");
    exitsim();
  }
  //printf("Cond Var Init successful.\n");
  return SYNC_SUCCESS;
}
...
cond_t CondCreate(lock_t lock) {
  ...
  if(lock < 0)
  {
    printf("Lock not initialized. Cond var allocation fail.\n");
    return SYNC_FAIL;
  }
  intrval = DisableIntrs();
  for(cond = 0; cond < MAX_CONDS; cond++)
  {
    ...
    {
      conds[cond].inuse = 1;
      conds[cond].condVarLock = lock;
      conds[cond].waitCount = 0;
      break;
    }
  }
  ...
  //printf("CondInit : conds[%d].inuse = %d conds[%d].condVarLock = %d\n",cond,conds[cond].inuse,cond,conds[cond].condVarLock);
  if(cond == MAX_CONDS)
  {
    return SYNC_FAIL;
  }
 ...
 // printf("OS : Cond Create successful. Cond variable handle = %d\n",cond);
  return cond;
}
...
int CondWait(Cond* cond)
{
  ...
  //printf("Entered the condWait function.\n");
  //printf("condVarLock = %d\n", cond->condVarLock);
  //printf("Reading Lock info.\n");
  //printf("locks[cond->conVarLock].pid =%d Current pid = %d\n",locks[cond->condVarLock].pid,GetCurrentPid());
  intrval = DisableIntrs();

  /*if(locks[cond->condVarLock].pid == GetCurrentPid() )
  {
    //printf("The process %d already has the lock.\n",GetCurrentPid());
    RestoreIntrs(intrval);
    return SYNC_SUCCESS;
  }*/
  
  //printf("Preparing Queue Init.\n");

  //if(locks[cond->condVarLock].pid != GetCurrentPid())
  
  //if(locks[cond->condVarLock].pid > 0)
  //{
  if((l = AQueueAllocLink((void*)currentPCB)) == NULL)
    {
      printf("Fatal Error : Cound not allocate link for condVar queue.\n");
      exitsim();
    }

   // printf("Cond Wait : Link address = %p\n",l);
   // printf("Addng %d to waiting queue.\n",GetCurrentPid());
    if(AQueueInsertLast(&cond->waiting,l) != QUEUE_SUCCESS)
    {
      printf("Failed to insert into waiting queue of condVar.\n");
      exitsim();
    }
    //printf("Releasing lock.\n");
    
    cond->waitCount += 1;
    //printf("cond->waitingCOunt = %d\n",cond->waitCount);
    LockRelease(&locks[cond->condVarLock]);
    //printf("Sleeping process with current saved frame -> %p\n",currentPCB->currentSavedFrame);printf(currentPCB->name);printf("\n");
    ProcessSleep();
  //}
  //else
  //{
    LockAcquire(&locks[cond->condVarLock]);
  //}
...
int CondHandleWait(cond_t c) {
  // Your code goes here
  //printf("CondHandleWait : Entering now.\n");
  if(c < 0)
    return SYNC_FAIL;
  ...
  if(conds[c].condVarLock < 0)
    {
      printf("Lock not acquired by the process.\n");
      return SYNC_FAIL;
    }
...
int CondSignal(Cond* cond)
{
  ...
  //{
    if(!AQueueEmpty(&cond->waiting))
    {
      ...
      //printf("CondSIgnal : Link Addresss = %p\n",l);
      pcb = (PCB*)AQueueObject(l);
      if(AQueueRemove(&l) != QUEUE_SUCCESS)
      {
        printf("FATAL Error : could not remove link from cond var waiting queue.\n");
        exitsim();
      }
      cond->waitCount -= 1;
      //printf("cond->waitingCOunt = %d\n",cond->waitCount);
      //printf("Waking up process with current savned frame = %p\n",pcb->currentSavedFrame);printf(pcb->name);printf("\n");
      ProcessWakeup(pcb);
      //LockRelease(&locks[cond->condVarLock]);
    //}
  }
...
int CondBroadcast(Cond* cond)
{
  ...
  //if(cond->waitCount > 0)
    ...
    while(1)
    {
      ...
      l = AQueueFirst(&cond->waiting);
      //printf("Cond Broadcast : Waking up Link Address : %p\n",l);
      pcb = (PCB*)AQueueObject(l);
      if(AQueueRemove(&l) != QUEUE_SUCCESS)
      {
        printf("FATAL Error : could not remove link from cond var waiting queue.\n");
