...
void main (int argc, char *argv[])
{
  ...
  if (argc != 4) { 
    Printf("Usage: "); Printf(argv[0]); Printf(" <handle_to_shared_memory_page> <handle_to_page_mapped_semaphore> <number of h20 molecules>\n"); 
    Exit();
  } 
  ...
  while(i < num_h2o_molecules)
  {
    int j = 0;
    sem_signal(kryp->molecule_h2o);
 ...
 // Printf("Injected %d H2O molecules\n", num_h2o_molecules);
