...
/*  lock = lock_create();
  condVar = cond_create(lock);
  lockRetVal = lock_acquire(lock);
  condRetVal = cond_wait(condVar);
  lockRetVal = lock_release(lock);
  lockRetVal = lock_acquire(lock);
  condRetVal = cond_signal(condVar);
  lockRetVal = lock_release(lock);
...
//  Printf("Creating %d processes\n", numprocs);
  ...
  cBufPtr->c_notFull = cond_create(cBufPtr->lock);
  cBufPtr->c_notEmpty = cond_create(cBufPtr->lock);

  //Printf("condVar crated for fullslots = %d\n and Condvar created for emptyslots = %d\n",cBufPtr->c_notFull,cBufPtr->c_notEmpty );
